...
def transcribe_with_groq(client: Groq, audio_path: Path):
    """
    呼叫 Groq Whisper API 進行轉錄，取得帶時間軸的 segment。
    """
    print(f"正在呼叫 Groq Whisper API (whisper-large-v3-turbo) 進行轉錄...")
    with open(audio_path, "rb") as f:
        # Check if file size > 25MB? Groq limit is 25MB mostly.
        # But let's assume valid.
        try:
            response = client.audio.transcriptions.create(
                model="whisper-large-v3-turbo",
                file=(audio_path.name, f.read()), # file tuple (name, content)
...
def process_and_summarize(client: Groq, transcription_response, video_name: str = ""):
    """
...
def generate_quiz(client: Groq, processed_data: dict, video_name: str = ""):
    """
    使用 LLM 根據教材內容自動出題。
...
6. **禁止出題類型**：嚴禁詢問「影片第幾秒出現什麼」、「某時間點的畫面是什麼」這類無意義的記憶題。題目請專注於「知識內容」、「操作步驟」與「觀念理解」。
...
<script>
const quizData = {json.dumps({**quiz_data, "video_name": video_name}, ensure_ascii=False)};
...
async function submitScore(score, total) {{
    try {{
        await fetch('/api/quiz/submit', {{
            ...
            body: JSON.stringify({{
                video_name: quizData.video_name,
                score: score,
...
def process_video(client, video_path: Path, output_base_dir: Path, temp_dir: Path):
    ...
    if json_file.exists():
        ...
        try:
            # 2. 轉錄 (Groq Whisper)
            transcription = transcribe_with_groq(client, audio_path)
            
            # 3. 翻譯與篩選重點 (Groq Llama 3)
    ...
    try:
        ...
        quiz_html_file = output_base_dir / f"{video_path.stem.replace(' ', '_')}_quiz.html"
...
def main():
    parser = argparse.ArgumentParser(description="Video Transcription and Quiz Generation (Groq)")
    ...
    if args.video:
        ...
        if not video_path.exists():
            ...
            return
        process_video(client, video_path, output_base_dir, temp_dir)
    else:
        # Default behavior
        video_dir = Path("Video")
        video_extensions = [".mp4", ".mkv", ".mov", ".avi"]
        if not video_dir.exists():
             print(f"目錄 {video_dir} 不存在")
             return
